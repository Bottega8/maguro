...
module Maguro
  ...
  def self.base_template(project)
    ...
    throw "The environment variable #{ORGANIZATION} must be set. This value must correspond to the BitBucket account. Set it by running the command:   'export #{ORGANIZATION}=the_org_name'" if ENV[ORGANIZATION].nil?
